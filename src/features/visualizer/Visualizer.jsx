...
function Visualizer() {
  ...
  useEffect(() => {
    ...
    // Mark merge edges
    const edgesWithMergeFlag = graphEdges.map((edge) => {
      const targetNode = graphNodes.find((n) => n.id === edge.target);
      const isMergeEdge = targetNode?.data?.isMerge || false;
      return {
        ...edge,
        data: {
          ...edge.data,
    ...
    setEdges(edgesWithMergeFlag);

    // Update HEAD pointer position
    const headNode = nodesWithSelection.find((n) => n.data.isHead);
    if (headNode) {
      const nodeWidth = headNode.width ?? headNode.measured?.width ?? 48;
      setHeadPosition({
        // center X of the commit circle (approximate using node width)
        x: headNode.position.x + nodeWidth / 2,
        // top of the node wrapper; vertical gap handled in overlay
        y: headNode.position.y,
      });
    }

...
export default function LiquidEther({
  ...
  resolution = 0.5,
  isBounce = false,
  // Use site theme colors by default; will be resolved from CSS variables
  colors,
  useThemeColors = true,
  style = {},
  className = "",
  ...
  useEffect(() => {
    ...
    // Resolve palette from CSS variables if requested
    function resolveThemeColors() {
      try {
        const root = document.documentElement;
        const styles = getComputedStyle(root);
        const c1 =
          styles.getPropertyValue("--primary-color").trim() || "#667eea";
        const c2 =
          styles.getPropertyValue("--accent-color").trim() || "#f093fb";
        const c3 =
          styles.getPropertyValue("--secondary-color").trim() || "#764ba2";
        return [c1, c2, c3];
      } catch (e) {
        return ["#667eea", "#f093fb", "#764ba2"];
      }
    }
    ...
    const paletteTex = makePaletteTexture(themeColors);
    ...
    class CommonClass {
      ...
      init(container) {
        ...
        // Allow clicks to pass through the canvas; we'll listen globally for pointer movement
    ...
    class MouseClass {
      ...
      init(container) {
        this.container = container;
        // Listen on window so interaction works even if the canvas/container has pointer-events: none
      ...
      onDocumentTouchStart(event) {
        if (event.touches.length === 1) {
          ...
          if (this.onInteract) this.onInteract();
          // Use clientX/clientY to align with getBoundingClientRect()
